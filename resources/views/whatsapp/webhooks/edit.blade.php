...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-black text-primary-900 dark:text-white flex items-center gap-2">
                <i class="fas fa-network-wired text-primary-500"></i> Edit Webhook
            </h2>
            <p class="text-xs text-primary-500 mt-1">Update webhook configuration for {{ $session['name'] ?? ('session ' . $session['id']) }}</p>
        </div>
    ...
    <div class="card p-6">
        <form method="POST" action="{{ route('whatsapp.webhooks.update', $session['id']) }}">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="url" name="webhook_url" id="webhookUrl" value="{{ old('webhook_url', $session['webhook_url'] ?? $canonicalUrl) }}" required class="flex-1 w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="https://pay.feedtancmg.org/api/whatsapp/webhook">
                    ...
                    <p class="text-[10px] text-primary-400 mt-1">Destination for POST requests sent by Wasender on events.</p>
                    @error('webhook_url') <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="text-[10px] text-gray-400 uppercase font-bold mb-1 block">Webhook Secret</label>
                    <div class="flex gap-2">
                        <input type="text" name="webhook_secret" id="webhookSecret" value="{{ old('webhook_secret') }}" class="flex-1 w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="{{ $session['webhook_secret'] ? str_repeat('•', min(24, strlen($session['webhook_secret']))) : 'whsec_...' }}">
                    ...
                    <p class="text-[10px] text-primary-400 mt-1">The current secret is shown as dots for security. Leave empty to keep it, or rotate to set a new one.</p>
                </div>
                <div class="flex items-center gap-2">
                    <input type="hidden" name="webhook_enabled" value="0">
                    <input type="checkbox" name="webhook_enabled" id="webhook_enabled" value="1" {{ old('webhook_enabled', $session['webhook_enabled'] ?? 0) ? 'checked' : '' }} class="w-4 h-4 rounded">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                        @foreach($events as $event)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="events[]" value="{{ $event }}" class="w-4 h-4 rounded" {{ in_array($event, old('events', $session['webhook_events'] ?? [])) ? 'checked' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (generateSecretBtn) {
            generateSecretBtn.addEventListener('click', function () {
                ajaxPost('{{ route('whatsapp.webhooks.generate-secret') }}', this, 'Rotating...', function (data) {
                    if (data.success) {
