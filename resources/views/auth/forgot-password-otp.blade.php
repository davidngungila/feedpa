...
<head>
    ...
    <title>Verify OTP | FEEDTAN</title>
        ...
        <div class="relative w-full max-w-md mx-4">
            <div class="card rounded-2xl p-8 shadow-2xl" :class="darkMode ? 'bg-[#0d1f16] border border-[#1a3328]' : 'bg-white'">
                ...
                <div class="text-center mb-8">
                    ...
                    <h1 class="text-2xl font-bold" :class="darkMode?'text-white':'text-primary-900'">Verify OTP</h1>
                    <p class="text-sm mt-1" :class="darkMode?'text-primary-300':'text-primary-600'">Enter the 6-digit OTP sent to your email</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.verify') }}">
                    @csrf
                    <div class="mb-6">
                        <label class="block text-sm font-semibold mb-2" :class="darkMode?'text-primary-300':'text-primary-700'">One-Time Password (OTP)</label>
                        <div class="relative">
                            <i class="fas fa-key absolute left-3 top-1/2 -translate-y-1/2 text-sm" :class="darkMode?'text-primary-400':'text-primary-500'"></i>
                            <input id="otp" type="text" name="otp" maxlength="6" required placeholder="000000" inputmode="numeric"
                                   class="w-full pl-10 pr-3 py-3 rounded-xl border text-center text-2xl font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all"
                ...
                <div class="mt-6 text-center flex items-center justify-center gap-2">
                    <p class="text-sm text-primary-500 dark:text-primary-400">Didn't receive OTP?</p>
                    <form method="POST" action="{{ route('password.email') }}">
                        @csrf
                        <input type="hidden" name="email" value="{{ session()->get('password_reset_email') }}">
                        <button type="submit" class="text-sm font-bold text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 transition-colors">Resend OTP</button>
                    </form>
                </div>

                <div class="mt-4 text-center">
                    <a href="{{ route('password.request') }}" class="text-sm font-semibold text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 transition-colors">
