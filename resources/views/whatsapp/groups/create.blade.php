...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-black text-primary-900 dark:text-white flex items-center gap-2">
                <i class="fas fa-users text-primary-500"></i> Create Group
            </h2>
            <p class="text-xs text-primary-500 mt-1">Create a new contact group</p>
        </div>
    ...
    <div class="card p-6">
        <form method="POST" action="{{ route('whatsapp.groups.store') }}">
            @csrf
            <div class="space-y-4">
                <div>
                    <label class="text-[10px] text-gray-400 uppercase font-bold mb-1 block">Group Name *</label>
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Sales Team">
                ...
                <div>
                    ...
                    <input type="text" name="group_id" value="{{ old('group_id') }}" class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="e.g. 255700000000-123456789@g.us">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Optional description">{{ old('description') }}</textarea>
                </div>
                <div>
                    <label class="text-[10px] text-gray-400 uppercase font-bold mb-2 block">Add Members</label>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-60 overflow-y-auto p-3 rounded-xl border border-primary-100 dark:border-primary-800">
                        @forelse($contacts as $contact)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="contacts[]" value="{{ $contact->id }}" class="w-4 h-4 rounded">
                                <span class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ $contact->name }}</span>
                            </label>
                        @empty
                            <p class="text-[10px] text-primary-400 col-span-full">No contacts yet. <a href="{{ route('whatsapp.contacts.create') }}" class="text-primary-600 underline">Add one</a></p>
                        @endforelse
                    </div>
                </div>
            </div>
