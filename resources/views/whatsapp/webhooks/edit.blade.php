...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-primary-500 mt-1">Update webhook configuration</p>
        </div>
    ...
    <div class="card p-6">
        <form method="POST" action="{{ route('whatsapp.webhooks.update', $webhook->id) }}">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                    <label class="text-[10px] text-gray-400 uppercase font-bold mb-1 block">Webhook Name *</label>
                    <input type="text" name="name" value="{{ old('name', $webhook->name) }}" required class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Message Status Updates">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="url" name="url" id="webhookUrl" value="{{ old('url', $webhook->url) }}" required class="flex-1 w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="https://your-domain.com/api/whatsapp/webhook/token">
                    ...
                    <input type="hidden" name="token" id="webhookToken" value="{{ old('token', $webhook->token) }}">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                        @foreach(['message_received', 'message_sent', 'message_delivered', 'message_read', 'session_status', 'contact_update', 'group_update'] as $event)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="events[]" value="{{ $event }}" class="w-4 h-4 rounded" {{ in_array($event, old('events', $webhook->events ?? [])) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="secret" id="webhookSecret" value="{{ old('secret', $webhook->secret) }}" class="flex-1 w-full px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="whsec_...">
                    ...
                    <p class="text-[10px] text-primary-400 mt-1">Used to verify webhook payloads sent to your endpoint. Leave empty to keep the current secret.</p>
                </div>
                <div class="flex items-center gap-2">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $webhook->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded">
